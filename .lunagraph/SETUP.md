...
**To load a specific canvas instead of all pages**, replace `initialPages` with `initialElements`:
...
- `snapshotMetadata` - Metadata for component viewing (enables double-click to see structure)
- `initialPages` - All pages from the pages manifest
- `initialElements` - (alternative) Canvas data for a single canvas
